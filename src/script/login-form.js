...
const passwordVisibilityBtn = loginForm.querySelector(".password-visibility");

const showPassword = (button) => {
  const parentElement = button.parentElement;
  const inputPassword = parentElement.querySelector(".input-form");
  const eyeIcon = button.querySelector(".eye");
  const isSeeing = eyeIcon.classList.contains("seeing");
...
passwordVisibilityBtn.addEventListener("click", () => {
  showPassword(passwordVisibilityBtn);
})
